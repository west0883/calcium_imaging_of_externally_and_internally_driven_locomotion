...
if verbose, fprintf('jade -> Whitening the data\n'); end
 ...
 %rangeW		= n-m:n-1			; % indices to the m  most significant directions
...
CM 		= gpuArray(zeros(m,m*nbcm));  % Storage for cumulant matrices
R 		= gpuArray(eye(m));  	%% 
Qij 		= gpuArray(zeros(m));	% Temp for a cum. matrix
Xim		= gpuArray(zeros(1,m));	% Temp
Xjm		= gpuArray(zeros(1,m));	% Temp
scale		= gpuArray(ones(m,1)/T) ; % for convenience
...
for im = 1:m        
	Xim = X(im,:) ;  % for up to the number of sources/ICs you want it to calculate 
...
if 1, 	%% Init by diagonalizing a *single* cumulant matrix.  It seems to save
	...
	[V,D]	= eig(CM(:,1:m));	% For instance, this one %%%% THE 1:m IS AN EXAMPLE SINGLE CUMULANT WITHIN ALL AND YOU CAN PROBABLY CHANGE THIS RANDOMLY 
