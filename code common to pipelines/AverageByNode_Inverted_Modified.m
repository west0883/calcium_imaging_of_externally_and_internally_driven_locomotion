...
function [parameters] = AverageByNode_Inverted_Modified(parameters)
...
%        
% 
%             % *** REMOVE THIS STEP FOR INVERTED *** 
%             % If Betas, remove intercepts from data
% %             if strcmp(output_type, 'BETA') 
% %     
% %                C = repmat({':'}, 1, ndims(data));
% %                C{1} = [1:numel(parameters.indices) + 1:size(data, 1)];
% %                data(C{:}) = [];
% %             
% %             end 
...
%             response_variable_number = size(data, 2);  %/ unique_corr_pairs;
% 
%             if rem(size(data,1), unique_corr_pairs) ~= 0
%                 error('Wrong number of correlations.')
%             end
%             
%             data = reshape(data, unique_corr_pairs, response_variable_number, size(data,3), size(data,4));
%             if parameters.multiply_by_average_sigma
...
%         end
    ...
    % this way). 
    %if PLSR_flag
        response_variable_number = size(node_averages, 3);
