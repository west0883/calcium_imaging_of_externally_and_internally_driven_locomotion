...
% tiffread, version 3.32 - November 23, 2018
%
% stack = tiffread;
...
while  ifd_pos ~= 0 
    ...
    if TIF.PlanarConfiguration == 1
        BytesPerPlane = TIF.SamplesPerPixel * 256 *256 *TIF.BytesPerSample; % IMG.width * IMG.height * TIF.BytesPerSample;
    else
    ...
    if isfield(TIF, 'MM_stack')
        ...
        if opt.SimilarImages && ~isempty(stack)
            if 256 ~= stack(1).width || 256 ~= stack(1).height %IMG.width 
        ...
        try
            if isfield(stack, 'info') && ~ isfield(IMG, 'info')
                IMG.info = stack.info;
            end
            stack = cat(2, stack, IMG);
        catch
